...
int32_t event_io_select_add(rtmp_event_io_t *io,rtmp_event_t *ev,uint32_t flag)
{
    ...
    if ((sctx->max_fd != -1) && (sctx->max_fd < c->fd)) {
        sctx->max_fd = c->fd;
    }
...
int32_t event_io_select_poll(rtmp_event_io_t *io, uint32_t msec, uint32_t flag)
{
    ...
    ready = select(sctx->max_fd + 1, &sctx->read_set, &sctx->write_set, 
                   NULL, tp);
    err = (ready == -1) ? sock_errno : 0;
...
int32_t event_select_repair_fd_sets(rtmp_event_io_t *io)
{
    ...
    sctx = io->data;
    for (s = 0; s < sctx->max_fd; s++) {

        if (FD_ISSET(s,&sctx->saved_read) == 0) {
            continue;
        }
    ...
    for (s = 0; s < sctx->max_fd; s++) {
        ...
        if (FD_ISSET(s,&sctx->saved_write) == 0) {
            continue;
        }        
...
{
    if (io->io_add) {
        return io->io_add(io,c->read,EVENT_READ);
    }
    return RTMP_FAILED;
}
